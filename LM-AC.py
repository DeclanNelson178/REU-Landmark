...
def load_data(size, num_lm):
    ...
    both_dataset = holder[:, 1:]
    both_labels = holder[:, 0]
    size = len(both_labels)
    attributes = len(both_dataset[0])

    tempAll_labels = np.zeros(len(both_labels))
    for i in range(len(both_labels)):
        if both_labels[i] == 'republican':
            tempAll_labels[i] = 0
        else:
            tempAll_labels[i] = 1
    ...
    for j in range(attributes):
        acceptable = []
        for i in range(size):
            if ((both_dataset[i][j] == 1) or (both_dataset[i][j] == 0)):
                acceptable.append(both_dataset[i][j])
        med = np.median(acceptable)
        medians.append(int(med))

    for i in range(size):
        for j in range(attributes):
            if (both_dataset[i][j] != 1) and (both_dataset[i][j] != 0):
                both_dataset[i][j] = medians[j]
    # todo remove this
    size = temp_subset
    m = size
    n = linear_dim0
